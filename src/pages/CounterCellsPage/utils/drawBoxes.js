...
export const drawBoxes = async (
    ...
    Object.values(outputCellCount).forEach(idList => {
        idList.forEach(id => registeredIdsSet.add(id));
    });
    ...
    for (let index = 0; index < yolo_json_data["xyxyn"].length; index++) {
        ...
        if (centerX >= lineStartX && centerX <= lineEndX) {
            if ((bboxY1 < middleY && bboxY2 > middleY) ||
                ...
                if (!registeredIdsSet.has(trackId)) {
                    try {
                        const imageDataUrl = await handleSaveRegions(imageSrc, [x1, y1, x2, y2]);
